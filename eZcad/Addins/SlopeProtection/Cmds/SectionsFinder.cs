...
[assembly: CommandClass(typeof(SectionsFinder))]

namespace eZcad.Addins.SlopeProtection
{
    [EcDescription("提取所有的横断面块参照的信息")]
    public class SectionsFinder
    {
        #region --- 命令设计
                    ...
                    else
                    {
                        if (promp)
                        {
                            var res = MessageBox.Show($"未找到记录横断面信息的块定义“{ProtectionOptions.BlockName_SectionInfo}”，" +
                                                      ...
                                                      $"\r\n是否继续提示", "提示", MessageBoxButton.YesNoCancel, MessageBoxImage.Information);
                            switch (res)
                            {
                                case MessageBoxResult.Yes: promp = true; break;
                                case MessageBoxResult.No: promp = false; break;
