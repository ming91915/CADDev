...
using eZcad.Addins.SlopeProtection.Entities;
using eZcad.Utility;
...
[assembly: CommandClass(typeof(SpInfosGetter))]

namespace eZcad.Addins.SlopeProtection
{
    public class SpInfosGetter
    {
        ...
        public void ExportSlopeInfos(DocumentModifier docMdf, SelectionSet impliedSelection)
        {
            ...
            // 所有的断面
            var allMileages = SectionsFinder.GetAllMileages(docMdf);
            ...
            var allMileInfos = allMileages.Select(r => new MileageInfo(r, MileageInfoType.Located, 0)).ToArray();
            // 所有设置的防护形式
            ...
            var allData = SlopeData.GetAllInfo(selectedSlopeDatas);
        ...
        {
            var slopes = SpInfosSetter.GetSlopeLines(_docMdf.acEditor, onlyExisted: true);
            ...
            IList<MileageInfo> allMileInfos)
        {
            ...
            foreach (var r in sp_LeftStyles)
            {
                ...
                {
                    mi = new MileageInfo(r.Mileage, MileageInfoType.Measured, r.SlopeLengthBelowFillTop);
                }
                else
                {
        ...
        private List<MileageInfo> Sort_Interpolate(List<MileageInfo> measuredSlopes, IList<MileageInfo> locatedSlopes)
        {
        ...
        private List<SegmentData<double, double>> Category_Sumup(IList<MileageInfo> sortedSlopes)
        {
            var slopes = sortedSlopes.Select(r => new KeyValuePair<double, double>(r.Mileage, r.SpLength)).ToArray();
        ...
        private void ExportAllDataToTxt(IList<SlopeData> slopeDatas, string infoPath)
        {
