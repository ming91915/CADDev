...
namespace eZcad.Addins.SlopeProtection
{
    public class SlopeLine
    {
        ...
        public CenterAxis CenterLine { get; private set; }
        ...
        public SlopeData XData
        {
            get
            {
                ...
                {
                    // 从 AutoCAD 中读取
                    var xd = Pline.GetXDataForApplication(SlopeData.AppName);
                    if (xd != null)
                    {
                        try
                        {
                            _xData = SlopeData.FromResultBuffer(xd);
                        }
                        catch (Exception)
                        {
                ...
                // 设置为默认值
                return _xData ?? (_xData = new SlopeData());
            }
        ...
        public static SlopeLine Create(DocumentModifier docMdf, Polyline pline)
        {
            ...
            if (segs != null)
            {
                ...
                if (ca != null)
                {
                    var slp = new SlopeLine(docMdf, pline, segs, ca);
                    return slp;
                }
            }
            return null;
        ...
        private SlopeLine(DocumentModifier docMdf, Polyline pline, LineSegment2d[] slopeSegs, CenterAxis centerAxis)
        {
            _docMdf = docMdf;
            _ed = docMdf.acEditor;
            //
            Pline = pline;
            CenterLine = centerAxis;
        ...
        private static CenterAxis FindCenterAxisOnBounds(DocumentModifier docMdf, Polyline pl)
        {
            ...
            if (res.Status == PromptStatus.OK)
            {
                ...
                var centerLine = CenterAxis.Create(docMdf, minLine);
                return centerLine;
            }
            else
            {
        ...
        /// <param name="pl"></param>
        private static CenterAxis FindCenterAxisOnConnection(DocumentModifier docMdf, Polyline pl)
        {
            // 先找到与边坡线相连的路面线
            var roadSurfFlt = new[]
            {
            ...
            if (res.Status == PromptStatus.OK)
            {
                ...
                if (res.Status == PromptStatus.OK)
                {
                    ...
                    if (l != null)
                    {
                        ...
                        {
                            var centerLine = CenterAxis.Create(docMdf, l);
                            return centerLine;
                        }
                    }
                }
        ...
        {
            var yw = CenterLine.GetYFromElev(elev); // 水位标高所对应的几何Y坐标
        ...
        private double GetTopElevation()
        {
            ...
            return CenterLine.GetEleFromY(maxY);
        }
        ...
        private double GetBottomElevation()
        {
            ...
            return CenterLine.GetEleFromY(minY);
        }
        ...
        private void CalculateXData(SlopeData xdata)
        {
            xdata.CenterElevation = CenterLine.CenterEle;
            xdata.TopElevation = GetTopElevation();
            xdata.BottomElevation = GetBottomElevation();
            xdata.NaturalSurfElevation = CenterLine.FindNaturalSurfElevation();
            //
            ...
            //
            xdata.OnLeft = CenterLine.IsOnLeft(Pline);
            //
            xdata.MileageStr = CenterLine.Mileage;
            //
            xdata.CenterAxisHandle = CenterLine.Line.Handle;
            xdata.InfoBlockHandle = CenterLine.InfoBlock.Handle;
        }
        ...
        {
            Pline.XData = SlopeData.ClearValue();
        }
        ...
        public string DataInfo
        {
            get
            {
                var info = XData.Mileage.ToString("0.000") + ",\t"
                    + (XData.OnLeft ? "左侧" : "右侧") + ",\t"
