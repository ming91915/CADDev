...
namespace eZcad.Addins.SlopeProtection
{
    public partial class ProtectionStyleLister : Form
    {
        ...
        private SlopeLine CurrentSlope
        {
            get
            {
                var v = listBox_slopes.SelectedValue as SlopeLine;
                return v;
            }
            set
            {
        ...
        public ProtectionStyleLister(List<SlopeLine> slopeLines)
        {
            InitializeComponent();
            KeyPreview = true;
            ValueChanged = false;
        ...
        private static int SlopeMileageComparison(SlopeLine sl1, SlopeLine sl2)
        {
            ...
            {
                // 都在道路的同一侧则比较桩号
                return sl1.XData.Mileage.CompareTo(sl2.XData.Mileage);
            }
        }
        ...
        private void StyleBtnOnClick(object sender, EventArgs eventArgs)
        {
            ...
            foreach (SlopeLine sl in listBox_slopes.SelectedItems)
            {
                sl.XData.Style = CurrentStyle;
                //var slpDt = new SlopeData(CurrentStyle, CurrentIsFill, sl.CenterAxisLine.IsOnLeft(sl.Pline), sl.Mileage);
                //sl.XData = slpDt;
            }
            //
            SetCurrentStyleUI(_currentStyle);
        ...
        private void button_Clear_Click(object sender, EventArgs e)
        {
            ...
            if (res == DialogResult.OK)
            {
                ...
                foreach (SlopeLine slp in listBox_slopes.DataSource as List<SlopeLine>)
                {
