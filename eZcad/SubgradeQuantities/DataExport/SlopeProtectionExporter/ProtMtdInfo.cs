...
namespace eZcad.SubgradeQuantities.DataExport
{
    public partial class Exporter_SlopeProtection
    {
        ...
        private class ProtMtdInfo
        {
            ...
            public ProtMtdInfo(double startStation, double endStation, double area, string protectionName,
            ...
            public bool Merge(ProtMtdInfo nextProtMtdInfo)
            {
                // 桩号的包含
                if ((nextProtMtdInfo.StartStation > this.EndStation) || (nextProtMtdInfo.EndStation < this.StartStation)) return false;
                ...
                if (nextProtMtdInfo.Range == Range)
                {
                    ...
                    switch (Range)
                    {
                        case ProtectionRange.PartialSlopeSegs:
                            {
            ...
            public static object[,] ConvertToArray(List<ProtMtdInfo> protMthinfos)
            {
                if (protMthinfos.Count > 0)
                {
                    var col = protMthinfos[0].GetTableHeader().Length;
