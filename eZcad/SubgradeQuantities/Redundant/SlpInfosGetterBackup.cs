...
using eZcad.SubgradeQuantities.RedundantCmds;
...
namespace eZcad.SubgradeQuantities.RedundantCmds
{
    ...
    internal class SlpInfosGetterBackup
    {
        ...
        public void ExportSlopeInfos(DocumentModifier docMdf, SelectionSet impliedSelection)
        {
            ...
            // 所有的断面
            var allStations = StationsFinder.GetAllStations(docMdf);
            ...
            var allMileInfos = allStations.Select(r => new StationInfo<double>(r, StationInfoType.Located, 0)).ToArray();
            // 所有设置的防护形式
            ...
            // ---------------------------------------------------------------------------------------------------------------
            var m = new double[allStations.Length, 0];
            // 添加一些数据列
            m = m.InsertVector<double, double, double>(false, new[] { allStations, },
                new[] { -1f });
        ...
        private List<SlopeDataBackup> GetAllSlopeData()
        {
            ...
            if (slopes != null && slopes.Count > 0)
            {
                ...
                slopeDatas.Sort(comparison: ProtectionUtils.CompareStation);
                return slopeDatas;
            }
            return null;
        }
        ...
        private Array ConstructSheetData_SlopeProtection(IList<SlopeDataBackup> slopes, ProtectionStyle style,
            IList<StationInfo<double>> allMileInfos)
        {
            var spData = new List<StationInfo<double>>();
            //
            var sp_LeftStyles = slopes.Where(r => r.Style == style);

            foreach (var r in sp_LeftStyles)
            {
                StationInfo<double> mi;
                if (r.FillExcav && ProtectionOptions.ConsiderWaterLevel)
                {
                    mi = new StationInfo<double>(r.Station, StationInfoType.Measured, r.SlopeLengthBeThinFillTop);
                }
                else
                {
                    mi = new StationInfo<double>(r.Station, StationInfoType.Measured, r.SlopeLength);
                }
                spData.Add(mi);
            }
        ...
        private List<StationInfo<double>> Sort_Interpolate(List<StationInfo<double>> measuredSlopes,
            IList<StationInfo<double>> locatedSlopes)
        {
            ...
            if (slopes.Count > 0)
            {
                ...
                // 2、 去掉有测量值的定位断面
                var sortedSections = new Stack<StationInfo<double>>();
                double duplicateStation = double.Epsilon;
                for (int i = slopes.Count - 1; i >= 0; i--)
                {
                    var slp = slopes[i];
                    if (slp.Station == duplicateStation)
                    {
                        // 说明出现重复里程，此时从众多重复里程中仅保留有测量值的那个里程断面
                        if (slp.Type == StationInfoType.Measured)
                        {
                            // 替换掉原集合中的值
                    ...
                    else
                    {
                        ...
                        duplicateStation = slp.Station;
                    }
                }

                // 3、 进行插值，此时 sortedSlopes 中，较小的里程位于堆的上面，
                // 而且集合中只有“定位”与“测量”两种断面，并没有“插值”断面
                var allSections = new List<StationInfo<double>>();
                ...
                for (int i = 1; i < count; i++)
                {
                    ...
                    if (slp.Type != lastType)
                    {
                        ...
                        var interpSec = new StationInfo<double>(interpMile, StationInfoType.Interpolated, 0);
        ...
        /// <param name="slopes"></param>
        private List<StationInfo<double>> Sort_SumDuplicate(List<StationInfo<double>> slopes)
        {
            slopes.Sort(ProtectionUtils.CompareStation);
            // 排序后集合中可能会有重复的里程(比如一个里程中，将一种边坡防护分两级坡分开算)

            // 现在将排序后相同里程的数据进行相加
            var distinctSlopes = new List<StationInfo<double>>();
            StationInfo<double> lastSlope = slopes[0];
        ...
        private List<SegmentData<double, double>> Category_Sumup(IList<StationInfo<double>> sortedSlopes)
        {
            var slopes = sortedSlopes.Select(r => new KeyValuePair<double, double>(r.Station, r.Value)).ToArray();
