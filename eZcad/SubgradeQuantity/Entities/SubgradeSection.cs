...
namespace eZcad.SubgradeQuantity.Entities
{
    ...
    public class SubgradeSection
    {
        ...
        public static SubgradeSection Create(DocumentModifier docMdf, Line centerLine)
        {
            ...
            if (infoBlock != null)
            {
                var att = infoBlock.GetAttributeReference(ProtectionOptions.MileageFieldDef);
                string mileage = att?.TextString;

                if (mileage != null)
                {
                    var slp = new SubgradeSection(docMdf, centerLine, infoBlock, mileage);
                    return slp;
                }
            }
            return null;
        ...
        private SubgradeSection(DocumentModifier docMdf, Line line, BlockReference infoBlock, string mileage) : this(line)
        {
            _docMdf = docMdf;
            CenterLine = line;
            _infoBlock = infoBlock.Handle;
            _mileage = ProtectionUtils.GetMileageFromString(mileage).Value;
            //
        ...
        private bool FindRoadSurf(Extents3d extCenterAxis, out Polyline leftRoadSurf, out Polyline rightRoadSurf)
        {
            ...
            // 请求在图形区域选择对象
            var res = _docMdf.acEditor.SelectCrossingWindow(pt1: extCenterAxis.MinPoint, pt2: extCenterAxis.MaxPoint,
                filter: new SelectionFilter(filterType));
            if (res.Status == PromptStatus.OK)
            {
                var lines =
        ...
        private bool FindRetainingWall(Extents3d extSection, out Polyline leftRetainingWall, out Polyline rightRetainingWall)
        {
            ...
            // 请求在图形区域选择对象
            var res = _docMdf.acEditor.SelectCrossingWindow(pt1: extSection.MinPoint, pt2: extSection.MaxPoint,
                filter: new SelectionFilter(filterType));
            if (res.Status == PromptStatus.OK)
            {
                var lines =
        ...
        private bool FindSlopes(Extents3d extSection, out Polyline leftSlope, out bool leftFill, out Polyline rightSlope, out bool rightFill)
        {
            ...
            if (res.Status == PromptStatus.OK)
            {
                ...
                var lefts =
                    lines.Where(
                        l =>
                            l.Layer == ProtectionOptions.LayerName_Slope_Left_Fill ||
                            l.Layer == ProtectionOptions.LayerName_Slope_Left_Cut).ToArray();
                if (lefts.Length > 0)
                {
                ...
                var rights =
                 lines.Where(
                     l =>
                         l.Layer == ProtectionOptions.LayerName_Slope_Right_Fill ||
                         l.Layer == ProtectionOptions.LayerName_Slope_Right_Cut).ToArray();
                if (rights.Length > 0)
                {
                    // 从多个边坡线中搜索某个端点距离中轴线中心最近的那一条
            ...
            if (left)
            {
                if (xdata.LeftSlopeExists)
                {
                    ...
                    var retainingWall = xdata.LeftRetainingWallExists
                        ...
                        : null;
                    return new SlopeLine(_docMdf, pl, this, onLeft: true, isFill: xdata.LeftSlopeFill, retainingWall: retainingWall);
                }
            }
            else
            {
                if (xdata.RightSlopeExists)
                {
                    ...
                    var retainingWall = xdata.RightRetainingWallExists
                        ...
                        : null;
                    return new SlopeLine(_docMdf, pl, this, onLeft: false, isFill: xdata.RightSlopeFill, retainingWall: retainingWall);
                }
            }
            return null;
        }
